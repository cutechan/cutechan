...
gulp.task("smiles", () => {
  const allSmiles = fs.readdirSync("smiles").filter(s => s.endsWith(".png"));
  const normalSmiles = allSmiles.filter(s => /^[a-z0-9_]+\.png$/.test(s));
  const allSet = new Set(allSmiles);
  const pathsWithRetina = normalSmiles
    .filter(s => allSet.has(s.slice(0, -4) + "@2x.png"))
    .map(s => `smiles/${s}`);
  const pathsWithoutRetina = normalSmiles
    .filter(s => !allSet.has(s.slice(0, -4) + "@2x.png"))
    .map(s => `smiles/${s}`);

  const withRetina = gulp
    .src(["smiles/*@2x.png"].concat(pathsWithRetina))
    .pipe(spritesmith({
      imgName: "smiles.png",
      cssName: "smiles.css",
  ...
  return merge(withRetina, withoutRetina)
    ...
    .on("end", () => {
      const smileIds = normalSmiles.map(s => `"${s.slice(0, -4)}"`);

      const jsSmiles = smileIds.join(",");
      const jsModule = `
